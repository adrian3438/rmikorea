/* RMI scroll-driven 3D stage. Port of the old js/scene.js: same geometry, same camera
   story, same materials — but an ES module bound to one canvas instead of a global
   window.RMI3D, so React owns the lifetime and createScene() can be torn down. */
import * as T from 'three';

// One entry per scroll step. x/y/z is the look-at target, r the distance,
// th the yaw, ph the pitch. Adding or removing an entry changes STAGE_COUNT
// in components/Stage.jsx and the dot count it renders.
const STAGES = [
...
const frameM = mat(0x7b8083, 0.9, 0.42),      // galvanised structural steel
...
function cyl(r1, r2, h, m, x, y, z, rx, rz) {
  const o = new T.Mesh(new T.CylinderGeometry(r1, r2, h, 26), m);
  o.position.set(x, y, z); o.rotation.x = rx || 0; o.rotation.z = rz || 0;
  return o;
}
function boltRing(r, n, rb, m) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const b = new T.Mesh(new T.CylinderGeometry(rb, rb, 0.05, 6), m || boltM);
...
function pipe(a, b, r, m) {
  ...
  const o = new T.Mesh(new T.CylinderGeometry(r, r, d.length(), 18), m || pipeM);
...
function run(pts, r, m, flanged) {
  ...
  for (let i = 0; i < pts.length - 1; i++) {
    const A = new T.Vector3().fromArray(pts[i]), B = new T.Vector3().fromArray(pts[i + 1]);
    const d = new T.Vector3().subVectors(B, A);
    g.add(pipe(pts[i], pts[i + 1], r, m));
    const j = new T.Mesh(new T.SphereGeometry(r * 1.06, 16, 12), m || pipeM);
    j.position.copy(B); g.add(j);
    if (flanged !== false && i === 0) {
      const f0 = flange(r);
      f0.position.copy(A).addScaledVector(d.clone().normalize(), r * 2.2);
      orient(f0, d); g.add(f0);
    }
    if (flanged !== false && i === pts.length - 2) {
      const f1 = flange(r);
      f1.position.copy(B).addScaledVector(d.clone().normalize(), -r * 2.2);
      orient(f1, d); g.add(f1);
    }
  }
  return g;
}
function valve(x, y, z, s, rot) {
  ...
  const f = flange(0.13 * s); f.position.y = -0.14 * s; g.add(f);
...
function gauge(x, y, z, ry) {
  ...
  g.add(cyl(0.115, 0.115, 0.012, glassM, 0, 0.032, 0, Math.PI / 2));
...
function jbox(x, y, z, s) {
  ...
  [-0.09, 0.09].forEach((o) => { g.add(cyl(0.035 * s, 0.035 * s, 0.1 * s, boltM, o * s, -0.24 * s, 0)); });
...
function motor(len, r, m) { // finned electric motor
  ...
  for (let i = -len / 2 + 0.06; i < len / 2; i += 0.075) g.add(cyl(r * 1.1, r * 1.1, 0.02, m, i, 0, 0, 0, Math.PI / 2));
...
function buildModel() {
  ...
  legs.forEach((x) => { g.add(box(0.16, 0.18, 3.4, frameM, x, 1.08, 0)); });
  g.add(box(9.2, 0.05, 3.4, deckM, 0, 1.0, 0));
  for (let gx = -4.3; gx < 4.4; gx += 0.28) g.add(box(0.02, 0.055, 3.38, deckM, gx, 1.03, 0)); // grating bars

  // ================= left filter / vessel module (own frame, in front) =================
  const mod = new T.Group();
  [-1.0, 1.0].forEach((x) => {
    [-0.95, 0.95].forEach((z) => {
      mod.add(box(0.14, 1.0, 0.14, frameM, x, -0.5, z));
      mod.add(box(0.28, 0.05, 0.28, darkM, x, -0.98, z));
    });
  });
  mod.add(box(2.45, 2.5, 2.2, vesselM, 0, 1.3, 0));
  mod.add(box(2.5, 0.1, 2.26, darkM, 0, 2.58, 0));
  mod.add(box(2.5, 0.1, 2.26, darkM, 0, 0.04, 0));
  for (let bz = -0.95; bz <= 0.95; bz += 0.38) {
    mod.add(cyl(0.04, 0.04, 0.06, boltM, 1.24, 2.3, bz, 0, Math.PI / 2));
    mod.add(cyl(0.04, 0.04, 0.06, boltM, 1.24, 0.35, bz, 0, Math.PI / 2));
  }
  ...
  [-1.25, -0.15].forEach((x, i) => {
    ...
    [0, 1, 2].forEach((k) => { c.add(cyl(0.032, 0.032, 0.025, k === 1 ? redM : glassM, -0.26 + k * 0.16, 1.14, 0.49, Math.PI / 2)); });
  ...
  for (let tx = -1.9; tx < 0.6; tx += 0.22) g.add(box(0.02, 0.1, 0.14, boltM, tx, 2.3, -0.2));

  // ================= process bay: vessels, pumps, valves =================
  [[1.5, 0.5, 1.7, oliveM], [2.7, 0.42, 1.45, vesselM]].forEach((v) => {
    const yb = 1.03;
    ...
    const dome = new T.Mesh(new T.SphereGeometry(v[1], 24, 14, 0, Math.PI * 2, 0, Math.PI / 2), v[3]);
  ...
  const m1 = motor(0.95, 0.36, pumpM); m1.position.set(0.5, 0.55, 0); pump.add(m1);
  ...
  const volute = new T.Mesh(new T.SphereGeometry(0.4, 22, 16), pumpM);
  ...
  const sf = flange(0.28); sf.rotation.z = Math.PI / 2; sf.position.set(-1.06, 0.58, 0); pump.add(sf);
  ...
  const m2 = motor(0.6, 0.24, oliveM); m2.position.set(0.3, 0.38, 0); p2.add(m2);
  ...
  [[3.9, 1.5, -0.2], [3.9, 1.5, 0.5]].forEach((v) => {
  ...
  for (let k = -4.4; k <= 4.4; k += 1.47) g.add(pipe([k, 1.06, -1.72], [k, 2.14, -1.72], 0.045, frameM));
  const lad = new T.Group();
  lad.add(pipe([-0.2, 0, 0], [-0.2, 1.1, 0], 0.035, frameM));
  lad.add(pipe([0.2, 0, 0], [0.2, 1.1, 0], 0.035, frameM));
  for (let ry = 0.15; ry < 1.1; ry += 0.22) lad.add(pipe([-0.2, ry, 0], [0.2, ry, 0], 0.025, frameM));
...
function makePin(label, x, y, z) {
  const g = new T.Group();
  const s = new T.Sprite(new T.SpriteMaterial({ map: pinTexture(label), transparent: true, depthTest: false }));
  s.scale.set(1.3, 0.44, 1); s.position.set(0.82, 0.42, 0);
  const dot = new T.Mesh(new T.SphereGeometry(0.075, 14, 12), new T.MeshBasicMaterial({ color: 0xec3013, depthTest: false, transparent: true }));
  const ring = new T.Mesh(new T.RingGeometry(0.13, 0.155, 28), new T.MeshBasicMaterial({ color: 0xec3013, transparent: true, side: T.DoubleSide, depthTest: false }));
  g.userData = { ring, sprite: s, dot };
...
function envTexture() { // simple studio equirect: bright sky, soft box, dark floor
  ...
  grd.addColorStop(0, '#6f7a84'); grd.addColorStop(0.45, '#33373a');
  grd.addColorStop(0.52, '#161413'); grd.addColorStop(1, '#0a0909');
...
/**
 * Build the stage on `canvas` and start rendering.
 * Returns the handle Stage.jsx drives it with; call dispose() on unmount —
 * React remounts effects in dev, and a leaked renderer keeps its WebGL context.
 */
export function createScene(canvas, opts = {}) {
  let lateral = opts.lateral || 0;
  let progress = 0, pinAmt = 0;
  let dragTh = 0, dragPh = 0, dragging = false, px = 0, py = 0, spin = 0;
  let frameId = null, disposed = false;

  const cur = { x: 0, y: 1.9, z: 0, r: 30, th: 0.85, ph: 1.06 };
  const tgt = { x: 0, y: 1.9, z: 0, r: 30, th: 0.85, ph: 1.06 };
  ...
  shadowCam.left = -10; shadowCam.right = 10; shadowCam.top = 10; shadowCam.bottom = -10; shadowCam.far = 40;
  key.shadow.bias = -0.0009; scene.add(key);
  const rim = new T.DirectionalLight(0xec3013, 0.22); rim.position.set(-9, 5, -8); scene.add(rim);
  const fill = new T.DirectionalLight(0xa8bcd2, 0.32); fill.position.set(-6, 4, 9); scene.add(fill);

  const model = buildModel();
  model.traverse((o) => { if (o.isMesh) { o.castShadow = true; o.receiveShadow = true; } });
  scene.add(model);

  const floor = new T.Mesh(new T.PlaneGeometry(80, 80), new T.MeshStandardMaterial({ color: 0x080707, metalness: 0.0, roughness: 1 }));
  floor.rotation.x = -Math.PI / 2; floor.receiveShadow = true; scene.add(floor);
  const grid = new T.GridHelper(60, 60, 0x3d3735, 0x241f1f);
  grid.material.transparent = true; grid.material.opacity = 0.16; grid.position.y = 0.002; scene.add(grid);

  const pins = [makePin('LEAK — SEAL', 2.0, 1.7, 0.95), makePin('VIBRATION', 3.1, 1.6, 0.95), makePin('GASKET', 2.7, 2.5, -0.6)];
  ...
  // Drag-to-rotate. Listeners sit on window so a drag that leaves the canvas
  // still tracks, but only a pointerdown that lands on the canvas starts one.
  const onDown = (e) => {
  ...
  const onMove = (e) => {
    if (!dragging) return;
    dragTh += (e.clientX - px) * 0.006;
    dragPh -= (e.clientY - py) * 0.004;
  ...
  function loop() {
    ...
    const p = Math.max(0, Math.min(n, progress));
    const i = Math.min(n - 1, Math.floor(p)), f = p - i, e = f * f * (3 - 2 * f);
    ...
    const look = new T.Vector3(cur.x, cur.y, cur.z);
    camera.position.set(
    ...
    pins.forEach((pin, idx) => {
      ...
      const o = Math.max(0, Math.min(1, pinAmt * 1.2 - idx * 0.12));
      pin.userData.sprite.material.opacity = o;
      pin.userData.dot.material.opacity = o;
      const r = pin.userData.ring, ph = (t * 0.7 + idx * 0.3) % 1;
      r.material.opacity = o * (0.85 - 0.6 * ph);
      const sc = 1 + 1.6 * ph; r.scale.set(sc, sc, sc);
